...
print("test start")
train_input_path = '/opt/ml/input/data_edit/eval/images'
train_input_nobg_path = '/opt/ml/input/data_edit_nobg/eval/images'
path = Path(train_input_nobg_path)
path.mkdir(parents=True, exist_ok=True)

#background폴더 생성
os.makedirs(train_input_nobg_path, exist_ok=True)

#폴더안에 들어있는 이미지
for image in os.listdir(train_input_path):
    if image.startswith("."):
        continue
    img_path = os.path.join(train_input_path,image)
    ...
    #make output
    output_img_path = os.path.join(train_input_nobg_path,image)
